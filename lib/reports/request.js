...
function process(data, callback) {
    ...
    log('data length before', data.length);
    ...
    //tableStart = data.indexOf('</tbody>', tableStart);
    //tableStart = data.indexOf('<tbody>', tableStart) + 8;

    //data = data.match('<div id="ReportGriddiv">\s*<table[^>]*>(.*)');
    log('table start', tableStart, 'table end', tableEnd);
