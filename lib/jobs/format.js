...
function format_description(desc) {
    ...
    var j1 = desc.split(/(About the Team)|(Primary Responsibilities)|(The Ideal candidate)|(The Job)|(Job Description)|(About the Role)|(About the job)|(About the Job)|(About theJob)|(ABOUT THE JOB)|(Core Responsibilities)|(Responsibilities)|(REQUIREMENTS)|(About You)/);
    var j2 = [];
    var d = [];
...
function jobData(raw_data) {
    ...
    raw_data.job.forEach(function(job) {
        ...
        if (job.location != 'va' && job.location != 'sf') {
            var clone = {};
            job.forEach(function(property, name){
                clone[name] = property;
            });

            job.location = 'va';
            clone.location = 'sf';
            clone.id = clone.id + '-sf';
            job_array.push(clone);
        }
    ...
    job_array.forEach(function(job) {
        ...
        if (data.all_urls[urls.long_url]) {
            if (!duplicates[job.id]) {
                log('Duplicate:', job.id, 'of', data.all_urls[urls.long_url], urls.long_url);
            ...
            var descriptionRaw = (job.description && job.description.trim) ? job.description : (job.briefdescription && job.briefdescription.trim) ? job.briefdescription : false;
            if (descriptionRaw) {
                job.description = format_description(descriptionRaw);
            } else {
                log('WARN', job.id, 'Missing Description', job);
            }

            // Critical Job
            if (job.requisitionId && job.requisitionId.length) {
    ...
    data.all_jobs.forEach(function(location, location_key){
        ...
        data.all_jobs[location_key].forEach(function(team, team_key) {
            teams.push( { team: team_key, jobs: team.sort(sort_title) } );
        });
        data.all_jobs[location_key] = teams;
    });
